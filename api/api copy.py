...
class Americas_growth(BaseModel):
    question: float = Field(
        ...
        description=(
            ...
            "Extract ONLY the numeric percentage value, without % sign. "
...
class Usa_growth(BaseModel):
    question: float = Field(
        -1,
        description=(
            "USA sales growth percentage (Net Sales) overall sales growth percentage "
            "Extract ONLY the numeric percentage value, without the % sign.  any information related to that field"
            "If not found, output -1."
        ),
    )
...
class India_growth(BaseModel):
    question: float = Field(
        ...
        description=(
            ...
            "Extract ONLY the numeric percentage value, without  % sign. any information related to that field "
            "If not found, output -1."
        ),
    )

class Category_share_largest_pct(BaseModel):
    question: float = Field(-1, description="Largest category share of reported net sales (%)  any information related to that field,. Numeric only; else -1.")

class Scotch_share_pct(BaseModel):
    question: float = Field(-1, description="Scotch share of reported net sales (%). Numeric only  any information related to that field,; else -1.")

class Beer_share_pct(BaseModel):
    question: float = Field(-1, description="Beer share of reported net sales (%). Numeric only  any information related to that field,; else -1.")

class Tequila_share_pct(BaseModel):
    question: float = Field(-1, description="Tequila share of reported net sales (%). Numeric only  any information related to that field,; else -1.")

class Vodka_share_pct(BaseModel):
    question: float = Field(-1, description="Vodka share of reported net sales (%). Numeric only  any information related to that field,; else -1.")

# Price-tier shares
class Price_tier_share_Value_pct(BaseModel):
    question: float = Field(-1, description="Value tier share of reported net sales (%). Numeric only  any information related to that field,; else -1.")

class Price_tier_share_Standard_pct(BaseModel):
    question: float = Field(-1, description="Standard tier share of reported net sales (%). Numeric only ,  any information related to that field,; else -1.")

class Price_tier_share_Premium_pct(BaseModel):
    question: float = Field(-1, description="Premium tier share of reported net sales (%). Numeric only;   any information related to that field, else -1")

class Price_tier_share_Super_premium_pct(BaseModel):
    question: float = Field(-1, description="Super-premium tier share of reported net sales (%). Numeric only; else -1. any information related to that field")
...
group_fields = {
    "Region": [APAC_Net_sales, Europe_Net_sales, APAC_Net_sales,
                Europe_Net_sales, Global_Net_sales, Rest_of_World_Net_sales, Usa_growth, Asia_row_growth, China_growth, India_growth ,
                  Category_share_largest_pct , Scotch_share_pct , Scotch_share_pct, Beer_share_pct, Tequila_share_pct , Vodka_share_pct],
}
